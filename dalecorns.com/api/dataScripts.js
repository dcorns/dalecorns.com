...
module.exports = {
        ...
        isValidProfile(token, data.profileId, function (result) {
            if (result) {
                cg.dbDocFind({ profileId: data.profileId }, 'profiles', function (err, data) {
                    if (err)
                        cb(new Error('Unable to save Profile'), null);
                    data[0].about = aboutMe;
                    cg.dbDocReplace(data[0], 'profiles', function (err, data) {
                        if (err)
                            cb(new Error('Profile save failed'), null);
                        else
                            cb(null, data.result);
                    });
                });
    ...
    saveActivity: function saveActivity(data, cb) {
        ...
        if (data.id)
            cg.dbDocUpdate({ _id: data.id }, data.updates, 'currentActivities', function (err, data) {
                if (err)
                    cb(err, null);
                else
                    cb(null, data);
            ...
            cg.dbDocInsert({ activity: 'act' }, data.updates, 'currentActivities', function (err, data) {
                if (err) {
                    cb(err, null);
                }
                else {
    ...
    getCurrentIncomplete: function getCurrentIncomplete(data, cb) {
    }
};
